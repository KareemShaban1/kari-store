...
class OrdersController extends Controller
{
    ...
    public function changeOrdersStatus($cart_id, $status) {
        try {
            // 'pending','processing','delivering','completed','cancelled','refunded'
    ...
    public function orderReports($status = null)
    {
        ...
        return view('backend.dashboards.delivery.orders.orders_report', compact('orders', 'status'));
    }
    ...
    public function deliveredOrdersReport(){
        ...
        $query = Order::whereIn('id', $orderIds)->where('status','completed')->with('user', 'store');
          ...
          return view('backend.dashboards.delivery.reports.delivered_orders', compact('orders','deliveryOrders'));
